...
export class Visualizer {
  ...
  static drawGraphs(): void {
    ...
    // function allEnergy(room: any) {
    //   let allEnergy = 0;
    //   let structures = room.find(FIND_STRUCTURES, {
    //     filter: (structure: { structureType: string }) => {
    //       return structure.structureType == STRUCTURE_EXTENSION || structure.structureType == STRUCTURE_SPAWN;
    //     }
    //   });
    //   structures.forEach((t: { store: { getUsedCapacity: (arg0: string) => number } }) => {
    //     allEnergy += t.store.getUsedCapacity(RESOURCE_ENERGY);
    //   });
    // }
    // this.text(`能量`, { x: 1, y: 10 });
    // this.barGraph(Game.gcl.progress / new RoomVisual() , { x: 2.75, y: 9 });
  }
